...
class SubBleRepository {
    ...
    private val gattClientCallback: BluetoothGattCallback = object : BluetoothGattCallback() {
        ...
        private fun readCharacteristic(characteristic: BluetoothGattCharacteristic) {
            ...
            /*
            프로토콜 정의
            HD HR RR HC AT | 1H 1L | 2H 2L | 3H 3L | 4H 4L | -- -- -- -- -- CK ED
            02 3C 05 01 00 | 01 74 | 01 C0  |01 C0 | 01 C0 | 00 00 00 00 00 06 03
            */

            val hexString = hex

            // 16진수 문자열을 16진수 값으로 변환하여 배열에 추가
            val dataArray = mutableListOf<UByte>()

            for (i in 0 until hexString.length step 2) {
                val hex = hexString.substring(i, i + 2)
                val byteValue = hex.toUByte(16)
                dataArray.add(byteValue)
            }
